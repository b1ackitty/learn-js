...
;(() => {
  ...
  indicatorWrapper.addEventListener('click', (e) => {
    ...
    //    엘리먼트(요소)를 포함한 조상에서 전달된 선택자와 일치하는 가장 가까운 요소를 찾아서 반환
    const indicator = target.closest('.carousel__indicator')
    // 2. Element.matches(selector)
    //    엘리먼트(요소)가 전달된 선택자와 일치(매칭)하는지 확인해 불리언 값을 반환
    // const indicator = target.matches('.carousel__indicator')
    // if(target.matches('.carousel__indicator')) {
    //   indicator를 target으로 수정해서 아래 if 문 안에 있는 내용 넣으면 작동한다.
    // }
