...
;(() => {
  ...
  ;(() => {
    let foundIndexNumber = numbers.findIndex((n) => n === 13)
  ...
  ;(() => {
    let foundUser = users.find((user) => user.name === '나설')
  ...
  // 과목별 개수를 포함하는 객체 생성
  // 명령형 코드: for 문 사용
  // 선언적 코드: reduce() 메서드 사용
  ;(() => {
    // 타임테이블의 요일별 과목 배열 값들로 구성된 배열 생성
    const nestedSubjectArray = Object.values(timeTable)

    // 과목 배열 값으로 구성된 배열을 평탄화
    // 평탄화(Flatting)란?
    // [[1, 2], [3, 4]] -> [1, 2, 3, 4]
    const flattedSubjectArray = nestedSubjectArray.reduce((resultArray, subjectArray) => {
      return [...resultArray, ...subjectArray]
    }, [])
