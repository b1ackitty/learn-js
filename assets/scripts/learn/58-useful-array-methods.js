...
;(() => {
  ...
  console.log(numbers.findIndex((n) => n === 9)) // -1
...
;(() => {
  ...
  const hanyoung = people.find((person) => person.name === '한영')

  console.log(hanyoung)
})
...
;(() => {
  ...
  // 사람들 중 성이 박으로 시작하는 사람들(집합, 배열)을 거르고 싶어요!
...
// 용도별 함수 작성 방법 비교: 사용자 정의 함수 forEach() vs. map()
;(() => {
  const list = ['하나', '둘', '셋']
