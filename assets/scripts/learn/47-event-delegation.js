...
// 요소 집합 루프(반복, 순환)하여 이벤트 리스너 각각 추가
// 이벤트 리스너 5개 정의
;(() => {
...
;(() => {
  ...
  linkList.addEventListener('click', (e) => {
    ...
    console.log('e.target =', e.target) // 이벤트의 타겟(a 클릭하면 a, li 클릭하면 li)
  })
})

// 이벤트 위임 + 이벤트 대상 확인에 따른 조건 처리
// 단 1개의 이벤트 리스너 생성
// - 가장 가까운 공통의 부모에 이벤트 연결
// - 이벤트 버블링(기본)을 사용해 이벤트 위임(이벤트가 전파되기 때문)
;(() => {
  const linkList = document.querySelector('.link-list')
...
// 이벤트 대상(event target)이 일치하는지 검사
...
//    pointer-events: none 설정하여 마우스 포인트 이벤트에 영향을 받지 않도록 설정
;(() => {
  const linkList = document.querySelector('.link-list')
